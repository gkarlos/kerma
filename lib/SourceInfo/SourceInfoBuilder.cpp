...
SourceInfoBuilder::SourceInfoBuilder(const std::string &SourcePath,
  ...
  if (!fs::exists(SourcePath))
    throw std::runtime_error(SourcePath + " does not exist");
  if (fs::is_directory(SourcePath))
    throw std::runtime_error(SourcePath + " is a directory");
  ...
  if (RealSourcePath.empty())
    throw std::runtime_error(std::string("Error getting real path for ") +
                             SourcePath);
  /// The requested file must have an entry in the Compilation Database
  if (auto DBFiles = CompileDB->getAllFiles();
      std::find(DBFiles.begin(), DBFiles.end(), RealSourcePath) ==
      DBFiles.end())
